...
export default function TrainerUploader({
  ...
  if (scope === "existing_vendor") {
    return (
      <div className="flex items-center gap-2 min-w-0">
        {/* Gap 77: this select used to be an unlabelled field floating at the
            far right of the scope-tab row. It keeps its own label rather than
            relying only on the bar's "Ground" step marker, because "Vendor" is
            what the field holds and the step name is what it is for. */}
        <label
          htmlFor="trainer-grounding-vendor"
          className="text-xs font-semibold text-emerald-400 flex items-center gap-1.5 shrink-0"
        >
  ...
  return (
    ...
    >
      ...
      {/* Gap 77: the inline "Optional sample document:" / "Upload sample
          invoice:" caption that used to sit here is gone. TrainerControlBar
          now labels this whole segment ("2 Ground") and states the per-scope
          expectation beside it, so keeping a second caption here said the same
          thing twice in a row that was already too crowded to fit it. */}
