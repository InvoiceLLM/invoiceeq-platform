...
/**
 * Feature 10 — VendorFlowToggles.tsx
 *
...
export default function VendorFlowToggles({ role }: { role: string }) {
  const isAdmin = role === "Admin";

  const [settings, setSettings] = useState<VendorFlowState | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch("/api/settings/vendor-flow");
        if (!res.ok) throw new Error("Failed to load settings");
        const data: VendorFlowState = await res.json();
        setSettings(data);
        setSenderEmailDraft(data.outbound_sender_email ?? "");
      } catch {
        showToast("Failed to load vendor flow settings.", "error");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // --- Save helper ---
  const save = async (patch: Partial<VendorFlowState>) => {
    setIsSaving(true);
    try {
      const res = await fetch("/api/settings/vendor-flow", {
        method: "PUT",
  ...
  return (
    <>
      ...
      <div className="space-y-4">
        ...
        {/* Billing plan hint for pro_combined */}
        {settings.billing_plan === "pro_combined" && (
