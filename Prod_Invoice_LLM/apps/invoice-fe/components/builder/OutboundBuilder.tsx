 ...
 * existing one.
 *
 * FE Feature 22 Task 22.2 moved this out of `app/invoices/outbound-builder/page.tsx`
 * unchanged, so the same editor — and the same `handleCreate` — serves two hosts:
 * that page, and Records' `OutboundBuilderDrawer`. The only things a host decides
 * are where the header controls render (`Actions`: the page portals them into
 * Shell's header, the drawer draws them in its own bar), what it titles itself
 * (`onInvoiceNumberChange`) and how the error state is left (`onClose`).
 *
 * Reached only from an existing invoice (founder decision, 2026-09-04): the
 * outbound review page header and the outbound table row, both of which pass a
 * source only for VERIFIED/SENT/PAID/OVERDUE. There is deliberately no source
 * picker — the user chooses the source first, then builds.
 *
...
export interface OutboundBuilderProps {
  sourceId: string | null;
  /** Wraps the header controls (source link + Create). */
  Actions: ComponentType<{ children: ReactNode }>;
  /** The invoice number being built, for the host's title. */
  onInvoiceNumberChange?: (invoiceNumber: string | null) => void;
  /** When given, the error state offers Close instead of a link back to /invoices. */
  onClose?: () => void;
}
...
export default function OutboundBuilder({ sourceId, Actions, onInvoiceNumberChange, onClose }: OutboundBuilderProps) {
  ...
  const invoiceNumber = form?.invoice_number ?? null;
  useEffect(() => {
  ...
  if (loadError || !form || !defaults) {
    const leaveClass =
      "inline-flex items-center gap-1.5 rounded-lg border border-[#222D3D] px-3 py-1.5 text-xs font-semibold text-slate-300 transition hover:border-slate-500 hover:text-slate-100";
    return (
      <div
      ...
      >
        ...
        {onClose ? (
          <button type="button" onClick={onClose} className={leaveClass}>
            Close
          </button>
        ) : (
          <Link href="/invoices" className={leaveClass}>
