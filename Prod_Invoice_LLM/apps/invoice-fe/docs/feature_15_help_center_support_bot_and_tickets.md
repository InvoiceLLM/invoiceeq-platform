...
* **AI Support Chatbot Component (new):** `apps/invoice-fe/components/help/SupportChatWindow.tsx` — `SupportChatWindow`. Renders the SAGE conversation, prompt chips, the inline escalation card, and the `[ 🎫 Raise Ticket Directly ]` header button; posts to `/api/support/chat` and opens `SupportTicketModal` with pre-filled context.
