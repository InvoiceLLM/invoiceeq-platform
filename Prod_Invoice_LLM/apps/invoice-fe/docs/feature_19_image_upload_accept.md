...
| 19.5 | Playwright spec; **plus** a run against the dev stack with a real PNG confirming a `.pdf`-named row in the ledger. | **PARTIAL — this is why the tracker row is `[~]`.** Spec passes: `10 passed (31.5s)` (6 module + 4 browser, all API routes stubbed). The **real-PNG dev-stack run is NOT done**: it asserts BE Feature 28's conversion and `.pdf` filename rewrite, which is being built concurrently and is not deployed. Owed once BE 28 lands. **CLOSED 2026-09-14. Founder ruling 2026-09-14: local stack + real Azure DI/OpenAI accepted as the Azure-path evidence** in place of "the dev stack". Real `.png` **and** `.jpg` uploaded through `POST /api/v1/invoices/upload` against the local stack with real Azure Document Intelligence; both landed as `.pdf`-named `Invoice.file_path` rows carrying real PDF bytes and reached `AUDIT_REQUIRED`, and the FE Review Console rendered the extracted fields for the image-sourced invoice. Result lines: *"Both `.png` and `.jpg` fixture uploads → `.pdf`-named `Invoice.file_path`, real PDF bytes stored (Feature 28 core contract): PASS"* and *"FE Review Console loads and shows extracted fields for an image-sourced invoice: PASS"*. Evidence: `apps/invoice-be/docs/test_evidence/f28_image_upload_2026-09-14/`. **Caveat, not rounded up:** the `.pdf` name was read from Postgres and the API, and the invoice was opened in the Review Console — the `/history` ledger *list* was not separately screenshotted for this upload (it was, for the FE Gap 464 run the same session). The assertion this row exists for — BE Feature 28's conversion + filename rewrite observed on a real upload — is confirmed. |
