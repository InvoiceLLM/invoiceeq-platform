...
> **Superseded in part, 2026-08-17 — read [feature_14_trainer_redesign.md](feature_14_trainer_redesign.md) first for anything about rule *creation*.** Feature 14 (FE) / Feature 18 (BE) replaced how a rule comes into existence: rules are now created by correcting a real alert on a real invoice, behind a preview-before-commit gate, and Global-scope rule creation is removed entirely. The coordinates below are updated to reflect where things actually live now. Everything *else* in this doc — rule history, rollback, the workspace geometry, the plan gate, the P0/Gap sections — still applies.

* Trainer Page: [apps/invoice-fe/app/trainer/page.tsx](file:///c:/Users/S%20Banerjee/Desktop/Invoice_LLM/Prod_Invoice_LLM/apps/invoice-fe/app/trainer/page.tsx) — now also gates the route on `can_train` (FE Gap 232), not just billing plan
* Control Bar: [apps/invoice-fe/components/trainer/TrainerControlBar.tsx](file:///c:/Users/S%20Banerjee/Desktop/Invoice_LLM/Prod_Invoice_LLM/apps/invoice-fe/components/trainer/TrainerControlBar.tsx) — **Global section removed** (FE Gap 233); now Extraction Rules / Chat Response Style tabs plus the Correct Alerts / Ask Questions mode toggle
* Session entry (unified — vendor → real invoice picker, or PDF upload): `apps/invoice-fe/components/trainer/TrainerEntryPanel.tsx` *(new, FE Gap 234)*
* Alert list — the anchor for every correction: `apps/invoice-fe/components/trainer/AlertListPanel.tsx` *(new, FE Gap 235)*
* Correction forms: `apps/invoice-fe/components/trainer/AlertCorrectionModal.tsx` (unnecessary → tolerance / confidence-threshold / not-tunable explainer; wrong severity or message) and `apps/invoice-fe/components/trainer/FlagMissedAlertModal.tsx` (expected-but-absent alert) *(both new, FE Gap 235)*
* Preview-before-commit gate: `apps/invoice-fe/components/trainer/CommitModal.tsx` — **reworked** from a plain confirmation dialog into the structured-interpretation + historical-impact + explicit-confirm screen every correction path funnels through *(FE Gap 236)*
* QA chat (questions only; creates no extraction rule): `apps/invoice-fe/components/trainer/QaChatPanel.tsx` *(new, FE Gap 237)* — replaces `QnAPanel.tsx` on this screen
* Thumbs-down triage (shared with the main Chat screen): `apps/invoice-fe/components/chat/ThumbsDownTriage.tsx` *(new, FE Gap 238)*
* Chat Response Style panel: [apps/invoice-fe/components/trainer/ChatResponseStylePanel.tsx](file:///c:/Users/S%20Banerjee/Desktop/Invoice_LLM/Prod_Invoice_LLM/apps/invoice-fe/components/trainer/ChatResponseStylePanel.tsx) — unchanged; its endpoints are the same, but they now persist to the backend's `TenantChatSettings` table rather than the Global template row. **Note it sits under the Vendor branch, not Global** — FE Gap 221's entry saying otherwise is stale, and the tracker now carries a correction
* Rule History Drawer: [apps/invoice-fe/components/trainer/RuleHistoryDrawer.tsx](file:///c:/Users/S%20Banerjee/Desktop/Invoice_LLM/Prod_Invoice_LLM/apps/invoice-fe/components/trainer/RuleHistoryDrawer.tsx) — unchanged; still reads Global timelines, since committed Global rules remain live
* Document Viewer Panel: `apps/invoice-fe/components/trainer/PdfViewerPanel.tsx` — `scope` widened for `outbound`; `pdfUrl` is now always server-side for both entry paths
* Shared trainer API client: `apps/invoice-fe/lib/trainer-service.ts` — `startSession(scope, …)` replaced by `startSessionFromInvoice()` / `startSessionFromUpload()`
* Chat-correction API client: `apps/invoice-fe/lib/chat-training-service.ts` *(new)*
* **Orphaned by the redesign, still on disk** (documented follow-up, not deleted in that pass): `components/trainer/QnAPanel.tsx`, `components/trainer/TrainerUploader.tsx`, and `components/trainer/ScopeSelector.tsx` (the last was already unreferenced before it)
* Proxy Routes (**corrected 2026-08-01 — this section was badly stale**; updated again 2026-08-17): `app/api/trainer/sessions/[id]/chat/route.ts`, `app/api/trainer/sessions/[id]/commit/route.ts`, `app/api/trainer/templates/[id]/rollback/[version]/route.ts`, `app/api/trainer/templates/history/route.ts`, `app/api/trainer/upload/route.ts`, `app/api/trainer/vendors/route.ts`, `app/api/trainer/chat-style/route.ts`, `app/api/trainer/sessions/[id]/commit-behavior/route.ts`, `app/api/trainer/sessions/[id]/mode/route.ts`.
  * **Added 2026-08-17**: `app/api/trainer/sessions/from-invoice/route.ts`, `app/api/trainer/sessions/[id]/pdf/route.ts` (binary stream, not `proxyJson`), `app/api/trainer/sessions/[id]/preview/route.ts`, `app/api/trainer/sessions/[id]/corrections/{tolerance,confidence-threshold,alert-override,missed-alert}/route.ts`, `app/api/trainer/alert-types/route.ts`, plus the chat lane's `app/api/chat/messages/[messageId]/triage/route.ts`, `.../triage/source-verdict/route.ts` and `app/api/chat/rules/{,categories,preview,commit,[ruleId]}/route.ts`.
  * **Deleted 2026-08-17**: `app/api/trainer/sessions/from-production/route.ts` and `app/api/trainer/sessions/global/route.ts` — both backend endpoints now return 410 Gone.
