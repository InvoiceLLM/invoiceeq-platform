...
**Current Status:** 10 of 14 tracked features complete (Features 1–8, 10, 11); Feature 9 (Webhooks) and the three Service Flow extension features (2.1, 3.1, 4.1) remain spec-only. 0 open gap items remain — Gap 59 fixed 2026-07-29; Gaps 28 and 67 closed the same day as tracker entries, superseded by `feature_4_auditor.md` Tasks 4.9/4.10 (neither is actually built yet — see those tasks for real status). Feature 5 (Chat) completed 2026-07-22; Feature 6 (AI Trainer) frontend completed 2026-07-22, merged alongside its backend (Feature 10) 2026-07-24.
...
- `[ ]` [Feature 3.1: Service Flow — Send Invoices Tab — **NOVA**](feature_3.1_vendor_flow_ingestion.md) — spec only, added 2026-07-27; extends Feature 3
...
- `[x]` **Gap 28: No dedicated Invoices Queue / list page — closed 2026-07-29, superseded by `feature_4_auditor.md` Task 4.9.** Found while fixing Gap 26 (Sidebar's "Invoices" nav item and `RecentInvoicesTable`'s "View all ledger" link both pointed at the same nonexistent `/audit` route, removed rather than repointed since there was nowhere real to land). Fully designed as part of the Dashboard/Audit split (2026-07-29): Dashboard becomes overview-only (metrics + `NeedsAttentionWidget`), the full invoice table/filters relocate to a new `/invoices` queue screen, Sidebar's "Invoices" item re-added pointing there. Closing this Gap entry to avoid double-tracking — actual build status now lives on Task 4.9 (`feature_4_auditor.md`, assigned to Dev 1 in `docs/scratch/dashboard_audit_split_dev_briefs.md`), not here. **Not yet built** — this closes the gap-tracking entry, not the underlying code.
