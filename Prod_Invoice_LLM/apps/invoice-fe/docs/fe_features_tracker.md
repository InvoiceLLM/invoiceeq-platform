...
**Current Status:** 10 of 14 tracked features complete (Features 1–8, 10, 11); Feature 9 (Webhooks) and the three Service Flow extension features (2.1, 3.1, 4.1) remain spec-only. 3 open gap items remain (Gap 28, 59, 67). Feature 5 (Chat) completed 2026-07-22; Feature 6 (AI Trainer) frontend completed 2026-07-22, merged alongside its backend (Feature 10) 2026-07-24.
...
- `[ ]` **Gap 59: Outbound tab's autoplay silently freezes partway through** — found Jul 27, 2026, not yet fixed. See `feature_11_flows_visualization.md` Task 11.4 for the exact root cause and fix (drop `ob_rules` from the `sequence` array).
...
- `[ ]` **Gap 67: "Apply as standing rule" checkbox on the inbound Audit/correction screen** — flagged 2026-07-29, proposal only, not started. FE piece of `be_features_tracker.md` Gap 62 — add a checkbox to the invoice correction UI (wherever `PUT /audit/resolve/{id}` is called from) offering "apply this correction as a standing rule for this vendor?", wired to the new backend param that gap introduces. Backend enforces the safety re-extraction check before committing; FE just needs to surface the checkbox and show the result (rule applied / rule rejected because the safety check failed).
