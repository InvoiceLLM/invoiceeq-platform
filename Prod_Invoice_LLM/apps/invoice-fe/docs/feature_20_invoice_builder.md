...
| 20.6 (live) | End-to-end on the dev stack (Azure path): create → Sending ledger row appears → reaches `VERIFIED`. Evidence under `docs/test_evidence/f20_invoice_builder_<date>/`. | **Not run, and not claimed.** BE Feature 17's three endpoints were being built in parallel and had not landed, so there was nothing live to call — every `/api/**` route in the spec is stubbed with `page.route()`. This row stays open until the backend half ships. FE Gap 457, which blocked the final step of this scenario ("Sending ledger row appears"), is now closed — but that removes only the FE-side blocker; the row itself stays **not run** until BE Feature 17's endpoints exist. **CLOSED 2026-09-14. Founder ruling 2026-09-14: local stack + real Azure DI/OpenAI accepted as the Azure-path evidence** in place of "the dev stack". BE Feature 17's endpoints now exist and were called for real (no `page.route()` stubs): `build-defaults` → `build/preview` (200 `application/pdf`) → `build` (201) on seeded `VERIFIED` `IEQ-US-9003`, and the created row reached **VERIFIED** in Postgres with `sa_alerts=[]`. Result lines: *"Real clone through BE endpoints (build-defaults → build/preview → build) reaches **VERIFIED with zero alerts**: PASS"* and *"FE outbound-review page renders the built clone correctly: PASS"*. Evidence: `apps/invoice-be/docs/test_evidence/f17_invoice_builder_2026-09-14/`. **Caveat, not rounded up:** the middle step's wording, "Sending ledger row appears", was observed as the clone's own outbound review page rendering live (correct customer, incremented number, rolled dates, correct total) — the Sending *list* itself was not re-screenshotted live; that surface is the subject of FE Gap 457, closed, and is covered by `e2e/outbound-builder.spec.ts`. The standing-caveat hole below — "the fixtures encode the contract rather than observe it" — is what this run actually closes: the real backend answered the shapes the stubs assert. |
