...
* New component: `apps/invoice-fe/components/dashboard/OutboundInvoicesTable.tsx` — the outbound tab/half of the new unified Invoices/Audit queue screen built in `feature_4_auditor.md` Task 4.9. Mirrors that screen's server-pagination pattern (8 rows/page, `limit`/`offset`/`X-Total-Count`), reading `GET /outbound-dashboard/invoices` (see `feature_8.1_vendor_flow_dashboard.md`, BE) instead of `GET /invoices`. Status tabs use outbound's real lifecycle (`Verified`/`Needs Review`/`Sent`/`Paid`/`Overdue`), not inbound's. **Path corrected 2026-08-01**: lives under `components/dashboard/`, not `components/audit/` as originally written — confirmed via its real import site, `app/invoices/page.tsx`.
* New component: `apps/invoice-fe/components/dashboard/OutboundFilterBar.tsx` — customer-name dropdown (mirrors inbound's vendor dropdown, built from distinct `customer_name` values) + date range. No tag filter — outbound invoices don't carry `tags`. **Path corrected 2026-08-01**: also actually under `components/dashboard/`, same as above.
...
- `[~]` **BE Gap 531 (UI half): every outbound field the backend accepts is correctable** — built 2026-09-15. `CORRECTABLE_FIELDS` on `app/invoices/outbound-review/[id]/page.tsx` adds Vendor (issuer), PO Number, Currency, Invoice Discount, Discount % and Notes; a new "Additional Details" panel lists tax IDs, tax lines, payment instructions, references, addresses and compliance entries through the shared `components/audit/DetailListEditor.tsx` (specs in `lib/correctableDetails.ts`). Edits are staged into `corrections` and sent by the existing Save Correction call; a refused entry shows in the FE Gap 499 "Not saved" banner. `npx tsc --noEmit` clean for changed files; not run in a browser (founder tests manually). Full record: BE `be_features_tracker.md` Gap 531, FE `fe_features_tracker.md` under FE Gap 499.
- `[~]` **BE Gaps 537, 538, 535 (UI half): one alert per dismissal, server's alert list after a save** — built 2026-09-15. `OutboundAlertConsole.tsx` sends `alertRef(alert)` instead of the message and takes `remaining_alerts` from the response; the outbound review page sends `alerts.map(alertRef)` on Save Correction, keeps `remaining_alerts` (an alert raised by a correction stays visible) and shows the amber `resolveNotice` note. `npx tsc --noEmit` clean for changed files; not run in a browser.
- `[~]` **Change history panel (Admin only, with BE Gap 532)** — built 2026-09-15. The outbound review page now reads `useAuth()` and, for an Admin, renders `components/audit/ChangeHistoryPanel.tsx`: outbound corrections, Confirm Send and Mark Paid (with status change and notified staff), newest first, who and when. It reloads after Save Correction, Confirm Send and Mark Paid. `npx tsc --noEmit` clean for changed files; not run in a browser.
