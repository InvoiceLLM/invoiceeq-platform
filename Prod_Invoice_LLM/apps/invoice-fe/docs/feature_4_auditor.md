...
- [ ] **Task 4.9 (Gap 28): Build the unified Invoices/Audit queue screen.**
  - Build `app/invoices/page.tsx`, relocating `RecentInvoicesTable.tsx` + `FilterBar.tsx` here with their existing pagination/filter/tab behavior unchanged.
  - Re-add Sidebar's "Invoices" nav item, pointed at `/invoices`.
  - **Redundant-code removal**: once this page ships, remove `dashboard/page.tsx`'s own invoice-pagination state and fetch logic (`activeTab`, `currentPage`, `totalCount`, `fetchInvoicesPage`, `PAGE_SIZE`, `tabToStatusParams`) and its `RecentInvoicesTable`/`FilterBar` imports — see `feature_2_dashboard.md` Task 2.7 for the Dashboard-side half of this same cleanup, and `feature_2.1_vendor_flow_dashboard.md` Task 2.1.6 for the outbound-parallel note.
...
* **Task 4.9 verification**: confirm `/invoices` renders the same table/pagination/filter behavior that used to live on Dashboard, confirm Sidebar's "Invoices" item is active only on this route, confirm Dashboard no longer imports `RecentInvoicesTable`/`FilterBar` after the redundant-code removal step, and confirm no functional regression in Next/Previous paging or tab filtering (same underlying component, new location).
