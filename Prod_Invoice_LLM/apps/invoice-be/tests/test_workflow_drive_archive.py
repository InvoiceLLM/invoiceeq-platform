...
def test_approve_archives_to_drive_on_postgres():
    ...
    # BE Gap 477: the tenant the mock-auth (human) path resolves to on THIS
    # database, which is not necessarily MOCK_TENANT_ID -- see
    # `_mock_auth_tenant_id`. Resolved below, inside the session.
    tenant_id = None
    ...
    with Session(pg_engine) as pg_session:
        ...
        try:
            ...
            human_invoice = _seed_invoice(
                pg_session, tenant_id=tenant_id, invoice_number="PG-DRIVE-HUMAN"
            )
            key_invoice = _seed_invoice(
                pg_session, tenant_id=tenant_id, invoice_number="PG-DRIVE-KEY"
            )
            ...
            readonly_invoice = _seed_invoice(
                pg_session, tenant_id=tenant_id, invoice_number="PG-DRIVE-READONLY"
            )
