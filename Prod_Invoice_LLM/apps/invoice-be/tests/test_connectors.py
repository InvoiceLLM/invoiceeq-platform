...
def test_connectors_status_google_drive_only_on_postgres():
    ...
    with Session(pg_engine) as pg_session:
        ...
        app.dependency_overrides[get_db_session] = get_db_session_override
        # BE Gap 477: the row below has to belong to the tenant the mock-auth
        # request actually runs as. `get_tenant_context_allow_unpaid()` resolves
        # the mock identity by `clerk_user_id`, and on a real database that user
        # row already exists and carries its own `tenant_id` -- so seeding under
        # MOCK_TENANT_ID left `/connectors/status` reading a different tenant and
        # answering `Not Configured` forever. Ask the auth dependency, on this
        # session, which tenant the request will be scoped to.
        from dependencies import get_tenant_context_allow_unpaid

        auth_tenant_id = get_tenant_context_allow_unpaid(
