...
class _PromptRecordingLLM:
    """Minimal stand-in for the chat LLM that keeps every prompt it is given."""
    ...
    def with_structured_output(self, schema):
        ...
        class _Structured:
            def invoke(self, prompt):
                ...
                return outer.structured_results.pop(0)
...
def _ranked_chunks(count: int, tenant_invoice_ids: list = None) -> list[dict]:
    """Helper to return `count` chunk structures with simulated rank metadata."""
    chunks = []
    for rank in range(1, count + 1):
        invoice_id = str(tenant_invoice_ids[rank-1]) if tenant_invoice_ids else str(uuid4())
        chunks.append({
            "id": f"chunk-{rank}",
            "document": f"RANKMARKER-{rank} document text",
            "metadata": {
                "invoice_id": invoice_id,
                "vendor_name": f"Vendor {rank}",
                "page": rank,
            },
            "distance": 0.10 * rank,
            "keyword_score": 0,
...
def test_reorder_chunks_puts_the_best_first_and_the_second_best_last():
    """Feature 21 Phase 1b: ranks 1..5 must come out as 1, 3, 4, 5, 2 — the two
    strongest chunks at the two ends of the context, where an LLM attends most
    reliably, instead of rank 2 being buried mid-prompt ("lost in the middle")."""
    from agents.query_agent import _reorder_chunks_for_context

    chunks = _ranked_chunks(5)
    ordered = _reorder_chunks_for_context(chunks)

    assert [c["id"] for c in ordered] == ["chunk-1", "chunk-3", "chunk-4", "chunk-5", "chunk-2"]
    # The two endpoints are the point of the whole exercise; assert identity,
    # not just id equality.
    assert ordered[0] is chunks[0]
    assert ordered[-1] is chunks[1]
    # Reordering only — nothing dropped, duplicated, or mutated in place.
    assert sorted(c["id"] for c in ordered) == sorted(c["id"] for c in chunks)
    assert [c["id"] for c in chunks] == [f"chunk-{i}" for i in range(1, 6)]


@pytest.mark.parametrize("count", [0, 1, 2])
def test_reorder_chunks_leaves_short_lists_alone(count):
    """With 0 or 1 chunk there is nothing to move; with 2 the best is already
    first and the second-best already last, so the transform is a no-op."""
    from agents.query_agent import _reorder_chunks_for_context

    chunks = _ranked_chunks(count)
    assert [c["id"] for c in _reorder_chunks_for_context(chunks)] == [c["id"] for c in chunks]


def test_rag_prompt_reorders_context_but_keeps_citations_in_rank_order(db_session):
    """The route-level half of Phase 1b, plus 1a's mandate: the context blocks
    reach the prompt reordered, the RAG grounding mandate is in that same
    prompt, and the citation list the user sees stays in relevance order — the
    reordering is about where the model reads text, not about which source to
    show first."""
    from datetime import date
    from models import Invoice
    from agents import query_agent
    from agents.query_agent import _RAG_FAITHFULNESS_MANDATE

    invoice_ids = [uuid4() for _ in range(5)]
    for rank, invoice_id in enumerate(invoice_ids, start=1):
        db_session.add(Invoice(
            id=invoice_id, tenant_id=MOCK_TENANT_ID, file_path=f"rank{rank}.pdf",
            vendor_name=f"Vendor {rank}", invoice_number=f"INV-{rank}",
            grand_total=100.0 * rank, status="COMPLETED", invoice_date=date(2026, 1, 1),
        ))
    db_session.commit()

    chunks = _ranked_chunks(5, tenant_invoice_ids=invoice_ids)
    llm = _PromptRecordingLLM("Vendor 1 billed for freight.")

    with patch("agents.query_agent.get_cached_answer", return_value=None), \
         patch("agents.query_agent.set_cached_answer"), \
         patch("agents.query_agent.classify_query", return_value="RAG"), \
         patch("agents.query_agent.query_invoice_chunks", return_value=chunks), \
        ...
        result = query_agent.run_query_agent(
            str(uuid4()), "Which vendor billed us for freight?", str(MOCK_TENANT_ID), db_session
        )

    prompt = llm.prompts[0]
    positions = [prompt.index(f"RANKMARKER-{rank} ") for rank in range(1, 6)]
    order = [rank for _, rank in sorted(zip(positions, range(1, 6)))]
    assert order == [1, 3, 4, 5, 2]

    # 1a: the grounding mandate is actually in the prompt that was sent.
    assert _RAG_FAITHFULNESS_MANDATE in prompt
    assert "answer ONLY from the extracted document context chunks" in prompt
    assert "never speculate beyond the context" in prompt

    # Citations keep the retriever's own ranking.
    assert [c["invoice_id"] for c in result["citations"]] == [str(i) for i in invoice_ids]


def test_sql_summary_prompt_carries_the_faithfulness_mandate(db_session):
    """Feature 21 Phase 1a, SQL half: the summary prompt turns raw rows into
    English and is where an unbacked number would get invented, so the
    result-grounding mandate has to be in that prompt, not only the RAG one."""
    from agents import query_agent
    from agents.query_agent import _SQL_SUMMARY_FAITHFULNESS_MANDATE

    llm = _PromptRecordingLLM("Formatted summary.")
    llm.structured_results = [
    ...
    with patch("agents.query_agent.get_cached_answer", return_value=None), \
         ...
         patch("agents.query_agent._get_tenant_stats_summary", return_value=""), \
         patch("agents.query_agent.get_llm", return_value=llm):
        query_agent.run_query_agent(
            str(uuid4()), "How much did we spend last month?", str(MOCK_TENANT_ID), db_session
        )

    summary_prompt = llm.prompts[0]
    assert _SQL_SUMMARY_FAITHFULNESS_MANDATE in summary_prompt
    assert "summarize ONLY the query results shown below" in summary_prompt
    assert "never speculate beyond them" in summary_prompt


def test_validate_citations_accepts_a_well_formed_list_and_names_every_defect():
    """Feature 21 Phase 1c. Format/consistency only — this says nothing about
    whether the prose is actually backed by the cited page (Phase 3)."""
    from agents.query_agent import _validate_citations

    good = {"invoice_id": str(uuid4()), "vendor_name": "Blue Ridge Logistics", "page": 2}
    assert _validate_citations([good]) == []
    assert _validate_citations([]) == []

    problems = _validate_citations([
        good,
        {"invoice_id": str(uuid4()), "vendor_name": "Acme", "page": None},      # 1: no page
        {"invoice_id": "not-a-uuid", "vendor_name": "Acme", "page": 1},         # 2: bad id
        {"invoice_id": str(uuid4()), "vendor_name": "  ", "page": 0},           # 3: blank vendor + page 0
        {"vendor_name": "Acme", "page": 1},                                     # 4: no id at all
        "definitely not a citation",                                            # 5: not a mapping
    ])
    joined = "; ".join(problems)

    assert "citation[0]" not in joined  # the good one is never reported
    assert "citation[1]: missing page" in joined
    assert "citation[2]: invoice_id 'not-a-uuid' is not a UUID" in joined
    assert "citation[3]: page 0 is not a positive page number" in joined
    assert "citation[3]: missing vendor_name" in joined
    assert "citation[4]: missing invoice_id" in joined
    assert "citation[5]: expected a mapping, got str" in joined


def test_rag_route_warns_on_a_malformed_citation_without_failing_the_answer(db_session, caplog):
    """Phase 1c at the route level. Gap 239's existence check already guarantees
    the invoice_id resolves to a real row; a chunk whose metadata is missing
    `page` still slips through it and renders as "(Page None)", which reads to a
    user like a fabricated citation. It must be logged — and the answer must
    still be returned, since the invoice genuinely exists and the link works."""
    import logging
    from datetime import date
    from models import Invoice
    from agents import query_agent
    ...
    invoice_id = uuid4()
    db_session.add(Invoice(
        id=invoice_id, tenant_id=MOCK_TENANT_ID, file_path="pageless.pdf",
        vendor_name="Blue Ridge Logistics", invoice_number="BRL-77",
        grand_total=500.0, status="COMPLETED", invoice_date=date(2026, 1, 1),
    ))
    db_session.commit()

    # Page metadata absent — exactly what a partially-indexed chunk looks like.
    malformed_chunk = {
        "document": "freight charge line",
        "metadata": {"invoice_id": str(invoice_id), "vendor_name": "Blue Ridge Logistics"},
    }
    llm = _PromptRecordingLLM("Blue Ridge Logistics has freight charges.")
    ...
    with caplog.at_level(logging.WARNING, logger="agents.query_agent"), \
         ...
         patch("agents.query_agent.set_cached_answer"), \
         patch("agents.query_agent.classify_query", return_value="RAG"), \
         patch("agents.query_agent.query_invoice_chunks", return_value=[malformed_chunk]), \
         patch("agents.query_agent._get_tenant_stats_summary", return_value=""), \
         patch("agents.query_agent.get_llm", return_value=llm):
        result = query_agent.run_query_agent(
            str(uuid4()), "Which vendors have freight charges?", str(MOCK_TENANT_ID), db_session
        )

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("Malformed RAG citation" in w and "missing page" in w for w in warnings), warnings
    # Warn-only: the answer still ships, with the citation still attached.
    assert "Blue Ridge Logistics has freight charges." in result["content"]
    assert result["citations"][0]["invoice_id"] == str(invoice_id)


# ---------------------------------------------------------------------------
# Feature 21 Phase 2 — deterministic near-match fallback.
# ---------------------------------------------------------------------------

@pytest.mark.parametrize(("message", "expected"), [
    ("What did we buy from Blue Ridge Logistics?", "Blue Ridge Logistics"),
    ("How much did we spend on office supplies?", "office supplies"),
    ("Show all invoices", None),
])
def test_broadened_search_phrase_only_accepts_explicit_entity_or_category(message, expected):
    """A fallback may not turn a generic request into an unbounded tenant-row
    search; it needs an explicit entity/category phrase to be useful and safe."""
    from agents.query_agent import _find_broadened_search_phrase
    assert _find_broadened_search_phrase(message) == expected


def test_rag_empty_result_uses_one_broader_lookup_and_hedges_it(db_session):
    """Phase 2 must retry only a true RAG miss, preserve the same tenant argument,
    and label the resulting context as a possible match rather than an answer."""
    from datetime import date
    from models import Invoice
    from agents import query_agent
    ...
    invoice_id = uuid4()
    db_session.add(Invoice(
        id=invoice_id, tenant_id=MOCK_TENANT_ID, file_path="blue-ridge.pdf",
        vendor_name="Blue Ridge Logistics", invoice_number="BRL-1", grand_total=12.0,
        status="COMPLETED", invoice_date=date(2026, 1, 1),
    ))
    db_session.commit()
    chunks = [{
        "id": "candidate", "document": "BLUE RIDGE candidate document text",
        "metadata": {"invoice_id": str(invoice_id), "vendor_name": "Blue Ridge Logistics", "page": 1},
        "distance": 0.2, "keyword_score": 2, "matched_by": "keyword",
    }]
    llm = _PromptRecordingLLM("Did you mean Blue Ridge Logistics?")
    with patch("agents.query_agent.get_cached_answer", return_value=None), \
         patch("agents.query_agent.set_cached_answer"), \
         patch("agents.query_agent.classify_query", return_value="RAG"), \
         patch("agents.query_agent.query_invoice_chunks", side_effect=[[], chunks]) as lookup, \
         patch("agents.query_agent._get_tenant_stats_summary", return_value=""), \
         patch("agents.query_agent.get_llm", return_value=llm):
        result = query_agent.run_query_agent(
            str(uuid4()), "What did we buy from Blue Ridge Logistics?", str(MOCK_TENANT_ID), db_session
        )

    assert lookup.call_count == 2
    assert lookup.call_args_list[0].args == (str(MOCK_TENANT_ID), "What did we buy from Blue Ridge Logistics?")
    assert lookup.call_args_list[1].args == (str(MOCK_TENANT_ID), "Blue Ridge Logistics")
    assert "NOT an exact match" in llm.prompts[0]
    assert result["citations"][0]["invoice_id"] == str(invoice_id)


def test_rag_nonempty_result_does_not_retry_broader_lookup(db_session):
    """An eligible first-pass result wins; Phase 2 never spends another retrieval
    call or changes the normal successful RAG route."""
    from agents import query_agent
    llm = _PromptRecordingLLM("Answer")
    chunks = []
    with patch("agents.query_agent.get_cached_answer", return_value=None), \
         patch("agents.query_agent.set_cached_answer"), \
         patch("agents.query_agent.classify_query", return_value="RAG"), \
         patch("agents.query_agent.query_invoice_chunks", return_value=chunks) as lookup, \
         patch("agents.query_agent._get_tenant_stats_summary", return_value=""), \
         patch("agents.query_agent.get_llm", return_value=llm):
        query_agent.run_query_agent(str(uuid4()), "show invoices", str(MOCK_TENANT_ID), db_session)
    # Generic text gives no broadened phrase, so the original miss remains a miss.
    assert lookup.call_count == 1


# ---------------------------------------------------------------------------
# Feature 21 Phase 3 — RAGAS Faithfulness metrics
# ---------------------------------------------------------------------------

def test_rag_response_includes_contexts(db_session):
    """Verify that RAG route returns contexts list in its API response."""
    from agents.query_agent import run_query_agent

    invoice_id = uuid4()
    chunks = [{
        "id": "chunk1", "document": "Mocked RAG chunk text",
        "metadata": {"invoice_id": str(invoice_id), "vendor_name": "Mock Vendor", "page": 1},
        "distance": 0.1, "keyword_score": 1, "matched_by": "semantic"
    }]
    ...
    with patch("agents.query_agent.classify_query", return_value="RAG"), \
         patch("agents.query_agent.query_invoice_chunks", return_value=chunks), \
         patch("agents.query_agent.get_llm", return_value=MockLLM()):
        result = run_query_agent(str(uuid4()), "Show me invoices", str(MOCK_TENANT_ID), db_session)
        assert "contexts" in result
        assert result["contexts"] == ["Mocked RAG chunk text"]


def test_faithfulness_grader():
    """Verify grade_faithfulness correctly parses LLM output and scores claims."""
    from tests.benchmark.run_benchmark import grade_faithfulness

    class MockResponseClaims:
    ...
    class MockLLM:
        ...
        def invoke(self, prompt):
            ...
            if "JSON list of all individual" in prompt:
                # Claim extraction prompt
                return MockResponseClaims('["Invoice INV-123 is completed", "The total spend is $500"]')
            elif "directly inferred from the Context" in prompt:
                # Verification prompt
                if "INV-123 is completed" in prompt:
                    return MockResponseClaims("YES")
                else:
                    return MockResponseClaims("NO")
            return MockResponseClaims("NO")

    with patch("tests.benchmark.run_benchmark.get_llm", return_value=MockLLM()):
        score = grade_faithfulness(
            question="What is the status of INV-123 and total spend?",
            contexts=["Invoice INV-123 has completed processing."],
            answer="Invoice INV-123 is completed and the total spend is $500."
        )
        assert score == 0.5
