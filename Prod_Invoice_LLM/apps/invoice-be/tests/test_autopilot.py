...
def test_T20g_run_sync_stamps_run_identity_and_logs_empty_runs(db_session):
    ...
    # BE Gap 477: the downloaded bytes must sniff as a PDF. Feature 28 put
    # `normalize_upload()` at the door of the sync loop, and it resolves a
    # filename/bytes disagreement in favour of the BYTES -- so the old
    # `b"pdf-bytes"` fixture, written before that, was refused with
    # `Invalid file format: ACME March.pdf` and the run reported
    # `processed=0, failed=1`. The rest of the file already uses the
    # `b"%PDF-1.4 ..."` spelling; these two tests were simply never updated.
    with patch("services.autopilot_sync.get_valid_access_token", return_value="tok"), \
         patch("services.autopilot_sync.list_google_drive_files", return_value=remote), \
         patch("services.autopilot_sync.download_google_drive_file",
               return_value=b"%PDF-1.4 acme march"), \
...
def test_T21f_hidden_run_still_deduplicates(db_session):
    ...
    def _run_one(trigger):
        # BE Gap 477: real PDF magic bytes -- `normalize_upload()` sniffs the
        # content, not the name (see T20g above).
        with patch("services.autopilot_sync.get_valid_access_token", return_value="tok"), \
             patch("services.autopilot_sync.list_google_drive_files", return_value=remote), \
             patch("services.autopilot_sync.download_google_drive_file",
                   return_value=b"%PDF-1.4 hidden co"), \
             patch("services.autopilot_sync.upload_pdf_to_blob_storage", return_value="path/x.pdf"), \
             patch("services.autopilot_sync.charge_free_quota"), \
             patch("services.autopilot_sync._dispatch_queue"):
