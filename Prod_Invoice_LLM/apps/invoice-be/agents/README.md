...
* **Role**: Ingests raw unstructured layout text and visual documents, returning structured JSON matching a strict schemas definition.
* **LLM**: Azure OpenAI `gpt-4o` (or `gpt-4o-mini` for high-throughput, low-cost extraction).
* **LLM Parameters**:
...
### 2.2 Verification Agent (`verification_agent.py`)
* **Role**: Evaluates the semantic and mathematical validity of the extracted JSON.
* **LLM**: Azure OpenAI `gpt-4o` (only called to explain discrepancies; calculations are run locally).
* **LLM Parameters**:
  * `temperature`: `0.1` (low temperature to enforce logical consistency)
  * `max_tokens`: `2048`
* **Tools**:
  * `verify_line_items_math`: Checks if `sum(item_amount) == subtotal`.
...
* **Failure Actions**: If any validation check fails, the agent writes the warning comments and transitions the invoice status to `AUDIT_REQUIRED`.
