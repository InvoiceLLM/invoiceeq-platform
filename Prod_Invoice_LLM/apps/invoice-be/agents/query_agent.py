...
def _normalize_string_equality(sql: str) -> str:
    """Rewrite `column = 'value'`, `column IN ('v1','v2')`, and `column LIKE '%val%'`
    to case-insensitive, trimmed comparisons for OCR/LLM-sourced text columns, so
    exact-match or multi-vendor SQL generated by the LLM doesn't silently miss rows
    over incidental case/whitespace differences.

    BUG-CORE-03: extended to cover IN (...) and LIKE '...' clauses in addition to
    the original equality-only rewrite.
    """
    for column in _FUZZY_STRING_COLUMNS:
        # --- equality: column = 'value' ---
        eq_pattern = re.compile(rf"\b{column}\s*=\s*'([^']*)'", re.IGNORECASE)
        sql = eq_pattern.sub(
        ...
        # Rewrites each quoted value inside the IN list individually.
        def _rewrite_in(m: re.Match, col: str = column) -> str:
            inner = m.group(1)  # everything inside the parentheses
            # Replace each 'value' token inside the list with TRIM(LOWER('value'))
            inner_rewritten = re.sub(
                r"'([^']*)'" ,
                lambda mi: f"TRIM(LOWER('{mi.group(1)}'))",
                inner,
            )
            return f"TRIM(LOWER({col})) IN ({inner_rewritten})"

        in_pattern = re.compile(
            rf"\b{column}\s+IN\s*\(([^)]+)\)", re.IGNORECASE
        )
        sql = in_pattern.sub(_rewrite_in, sql)

        # --- LIKE: column LIKE '%value%' ---
        like_pattern = re.compile(rf"\b{column}\s+LIKE\s+'([^']*)'", re.IGNORECASE)
        sql = like_pattern.sub(
            lambda m, col=column: f"TRIM(LOWER({col})) LIKE TRIM(LOWER('{m.group(1)}'))",
            sql,
        )
    return sql
