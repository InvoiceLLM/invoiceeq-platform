...
# `payment_instructions` and `tax_ids` added by BE Gap 588 (CH-21), and this is the
# third place that ruling had to reach. Founder ruling 2026-09-16: nobody sees
# payment credentials in chat. `services/full_records.py` keeps them out of the
# full-record block and `_redact_credentials()` scrubs the prose -- but the
# `### Query Results` table is built here and appended to the answer AFTER
# redaction runs, deliberately (see `redact_query_internals`). So a query that
# simply selected the column -- and "payment terms for acme" links straight to it
...
"""
    ...
    if route == "RAG":  # a plain `if`, not `elif`: C3 re-dispatches here from the SQL branch
        ...
        # Only the local-fallback store counts as degraded, not "uninitialised".
        # `get_chroma_client_kind()` reports "uninitialised" until this process has
        # built a client, which is the normal state whenever the search itself is
        # stubbed -- treating it as an outage silenced the RAG route entirely
        # (found by tests/test_chat_sql_quality.py::test_persona_reaches_the_rag_prompt).
        rag_search_degraded = not chunks and get_chroma_client_kind() == "persistent-fallback"
