...
| Manual, Azure path | One phone photo of a real invoice uploaded through a stack backed by real Azure Document Intelligence reaches `COMPLETED`/`AUDIT_REQUIRED` with non-empty `ocr_text`, its `Invoice.file_path` is `.pdf`-named and the stored bytes are a real PDF, and the invoice's PDF **loads** in the viewer. Recorded under `docs/test_evidence/f28_image_upload_<date>/`. **Reworded on the founder's ruling, 2026-09-14 ("Reword the row").** The original wording also required that "`PdfViewerCanvas` shows the page with field highlights aligned". That clause is dropped, and the reason is a limitation of the harness rather than a judgement about the product: headless Chromium cannot screenshot the native PDF frame, so the one assertion the row hung on was the one no automated run could ever capture. What remains is everything this feature actually changes — the boundary conversion, the stored artefact, the terminal status, and that the converted PDF is loadable by the viewer. Highlight alignment is a `PdfViewerCanvas` property (FE), unaffected by this feature, and is not re-proved here. |
| Full suite | `uv run pytest -q` at the track checkpoint, per the narrow-then-full rule. (Was `--ignore=tests/us/run_chat_live_test.py`; the two live scripts were renamed to `live_chat_check.py` on 2026-09-14 under BE Gap 477, so no `--ignore` is needed.) |
...
| Manual, Azure path | One phone photo to `COMPLETED`/`AUDIT_REQUIRED` through real Azure DI, `.pdf`-named path, real PDF bytes, viewer loads the PDF (row reworded 2026-09-14) | **Not run, and not claimed.** No dev-stack run was performed in this session, so `docs/test_evidence/f28_image_upload_<date>/` does not exist. §7 makes this a pre-merge gate, so the tracker row stays `[~]` until it is done. **Founder ruling 2026-09-14: local stack + real Azure DI/OpenAI accepted as the Azure-path evidence** — the *environment* half of this row is therefore closed by the 2026-09-14 local-stack run below (`invoice_photo.png` + `invoice_photo.jpg` → `.pdf`-named `file_path`, real PDF bytes, `AUDIT_REQUIRED`, real Azure DI reading vendor/number/total). Result lines: *"Both `.png` and `.jpg` fixture uploads → `.pdf`-named `Invoice.file_path`, real PDF bytes stored (Feature 28 core contract): PASS"*, *"Reached `AUDIT_REQUIRED` (not stuck at UPLOADED/PROCESSING): PASS"*, and *"Viewer renders the PDF page with visible highlights aligned to fields: **NOT independently confirmed**"*. Evidence: `docs/test_evidence/f28_image_upload_2026-09-14/`. ~~**The row still does not pass**, because its own wording requires "`PdfViewerCanvas` shows the page with field highlights aligned" and that specific assertion is the one thing the run could not capture (headless-Chromium PDF-iframe limitation, not an observed defect). The tracker item stays `[~]` on that single item, nothing else.~~ **SUPERSEDED 2026-09-14 by the founder's second ruling of the day ("Reword the row"): the alignment clause is removed from §6's row** — see it there for the wording and the reason. Against the reworded row this run **PASSES**: `.pdf`-named `file_path`, real PDF bytes, `AUDIT_REQUIRED`, real Azure DI text, and the invoice PDF loads in the viewer (`review_console.png`). Highlight alignment is no longer asserted by this row, and was not observed to be broken — it was unobservable. |

### Recorded run — 2026-09-14 (local stack, real Azure Document Intelligence, NOT the dev stack)

`docs/test_evidence/f28_image_upload_2026-09-14/` — `invoice_photo.png` and `invoice_photo.jpg` uploaded via `POST /invoices/upload` against the local stack (real Azure DI, real Postgres). Both produced `.pdf`-named `Invoice.file_path` with real PDF bytes and reached `AUDIT_REQUIRED`; real Azure OCR correctly read vendor name, invoice number and grand total off the (mostly blank) photo fixture. FE Review Console loaded and rendered the extracted fields. **Not confirmed:** on-canvas highlight alignment in the PDF viewer — headless-Chromium's PDF-iframe renders blank in a Playwright screenshot (a known tooling limitation, not an observed app defect; the underlying blob and route were independently verified via curl and DOM inspection). This is a **local-stack substitute**, not the literal dev-stack/phone-photo row above — that row stays **not run** and the tracker item stays `[~]` for the reason §7 gives.
