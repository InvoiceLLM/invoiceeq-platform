...
| Manual, Azure path | One real clone on the dev stack from a previously `VERIFIED` outbound invoice: preview renders the source layout with the new number/date; after `build` the worker reaches `VERIFIED` with zero `builder_render_mismatch` alerts, and `PdfViewerCanvas` shows the new values in the old layout. Evidence under `docs/test_evidence/f17_invoice_builder_<date>/`. | **Not run, and not claimed.** No dev-stack session took place, so `docs/test_evidence/f17_invoice_builder_<date>/` does not exist. This is the only open row in this plan, and it is the same gap as FE Feature 20's `20.6 (live)` row seen from the other side — that one is now unblocked, since these endpoints exist. **Founder ruling 2026-09-14: local stack + real Azure DI/OpenAI accepted as the Azure-path evidence.** Run that day (local stack, real Azure Document Intelligence + Azure OpenAI): `GET build-defaults` → `POST build/preview` (200 `application/pdf`) → `POST build` (201) on seeded `VERIFIED` `IEQ-US-9003`. Result line: *"Real clone through BE endpoints (build-defaults → build/preview → build) reaches **VERIFIED with zero alerts**: PASS"*, with `builder_intent.render_mode="rerender"` and `sa_alerts=[]`. Evidence: `docs/test_evidence/f17_invoice_builder_2026-09-14/`. **Row now satisfied.** One nuance recorded honestly: the FE PDF-viewer panel rendered blank in the headless-Chromium screenshot (known Playwright PDF-iframe limitation), so "`PdfViewerCanvas` shows the new values" was not confirmed *visually* — but `verify_builder_readback` returning zero `builder_render_mismatch` alerts proves deterministically, from re-extracted PDF text, that the new values are on the page. |
...
| Manual, Azure path (amended) | Two real clones: one unchanged-count (substitute) and one with a row added (re-render). Both reach `VERIFIED` with zero `builder_render_mismatch` alerts. | **Not run, and not claimed** — same reason as the row above. Both clone shapes (unchanged count → substitute, row added → re-render) are covered end-to-end through the HTTP endpoints against Postgres, but not against the live dev stack with real OCR. **Amended 2026-09-14 — founder ruling: local stack + real Azure DI/OpenAI accepted as the Azure-path evidence.** The two-clone shape this row asks for is **obsolete**: BE Gap 462 deleted the substitution renderer and `plan_render_mode()`, so an unchanged-count clone is no longer a distinct path — every clone re-renders. The one live clone that exists is therefore the whole of what this row can still ask for, and it passed: `render_mode="rerender"`, VERIFIED, `sa_alerts=[]` (result line: *"`render_mode=\"rerender\"` confirms BE Gap 462's 'substitution deleted' design is what actually executes on a real clone: PASS"*). Evidence: `docs/test_evidence/f17_invoice_builder_2026-09-14/`. |
