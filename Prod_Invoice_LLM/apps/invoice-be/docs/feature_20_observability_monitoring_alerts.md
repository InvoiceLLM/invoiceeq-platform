...
### 2026-08-24 — Gap 300: the LLM call becomes an `AppDependencies` row (fixed, not deployed)

The gap, restated in one line: every outbound dependency this app makes was instrumented **except the model call**, so a dependency-time breakdown for the Chat and Ingestion & Extraction areas would either omit LLM time entirely or silently reclassify it as "app logic" (the residual between request duration and the sum of tracked dependencies). Confirmed live before the fix — `AppDependencies` in `law-invoicellm-dev` carried exactly five `DependencyType`s (`InProc`, `Azure queue`, `postgresql`, `HTTP`, `Azure blob`) and zero rows matching `openai`.

**Both options in the gap's own suggested fix were investigated against the real installed packages before choosing.**

*Option (a) — enable an OpenTelemetry OpenAI instrumentation in `main.py`'s `configure_azure_monitor()` setup — was rejected on evidence, not preference.* `pyproject.toml` and `uv.lock` were read rather than assumed: `azure-monitor-opentelemetry` 1.8.9 pulls in instrumentations for django, fastapi, flask, logging, psycopg2, requests, urllib and urllib3 — and nothing else. There is no `opentelemetry-instrumentation-openai`, no `-openai-v2`, no `openinference`, no `traceloop` anywhere in the lock. Adding one would mean a new dependency that pins its own `opentelemetry-instrumentation` (currently `0.64b0`, owned by the Azure distro) — and it patches the `openai` SDK client, so it would cover neither the `ollama` provider nor the `MockInvoiceLLM` fallback this app really runs on, and it would sit outside the "telemetry never raises" contract everything in `telemetry.py` follows.

*Option (b) — emit the dependency span by hand from `tracked_llm_call()` — was taken.* That wrapper is already at every real LLM call site (`agents/query_agent.py`, `agents/sage_orchestrator.py`, `agents/query_tools.py`, `agents/extraction_agent.py`, `agents/trainer_agent.py`, `routers/trainer.py`, `routers/dashboard.py`, `services/agent_eval.py`, `services/ops_digest.py`), so **no call site changed at all** and nothing new has to be remembered at the next one.

**What was built, all in `apps/invoice-be/telemetry.py`:**
- `resolve_gen_ai_system(llm, model)` → the `gen_ai.system` value: `az.ai.openai`, `ollama`, or `mock`. Reads the already-resolved model name first, because `resolve_model_name()` walks the `RunnableBinding` chain and reports `"mock"` for a `MockInvoiceLLM` answer — so a wrapped mock is caught, and a fabricated call can never be indistinguishable from a real one in a dependency rollup (the same rule that function already enforces for cost).
- `resolve_gen_ai_peer(gen_ai_system)` → the endpoint **hostname only**, which becomes `DependencyTarget`. Hostname only deliberately: the configured endpoint can carry a path and query string, and an Azure OpenAI URL's query string is where `api-version` (and in some SDK shapes, a key) travels. Empty for a mock.
- `_start_llm_dependency_span(...)` / `_end_llm_dependency_span(...)` → open/close one `SpanKind.CLIENT` span named `chat {model}` (the OTel semconv name for a chat completion) on tracer `invoice_be.telemetry.llm`, carrying `gen_ai.system`, `gen_ai.operation.name`, `gen_ai.request.model`, `peer.service`, `server.address`, plus non-semconv `agent_name`/`tenant_id`/`request_id`/`run_source` — `agent_name` specifically because `DependencyType` collapses every LLM call in the app to one value, and the per-feature-area breakdown this gap exists to unblock needs a grouping key in `customDimensions`. **`run_source` added 2026-08-24 (Gap 304 half 1)** for the same reason applied to a different question: eval/benchmark runs now export their own spans too (`golden`/`predeploy`), and without this attribute an eval turn's dependency row is indistinguishable from a real user's — so a production dependency-time breakdown must filter `run_source == "production"`. Verified to survive the exporter's own span→envelope conversion into `customDimensions`, not just present on the span. Token counts (`gen_ai.usage.input_tokens`/`output_tokens`, `llm_calls`) are set at close, since they are only known then. On failure the span's status is set to `ERROR`, which is what the exporter reads for the dependency's `Success` column.
- `tracked_llm_call()` now resolves the model name up front (the span needs it at *start* time) and closes the span just before emitting `llm_agent_call`, so the two surfaces report the same duration for the same call. The pre-existing `model or resolve_model_name(llm)` fallback is preserved verbatim in the `finally` if that up-front resolution ever came back empty.

**The span is started, not entered as the current span** — deliberately. Making it current would mutate the OpenTelemetry context across the `yield` of a contextmanager used at both sync and async call sites; starting it plainly still reads the current context for its **parent**, which is the only part that matters here.

**Failure contract, tested:** the start path returns `None` on any exception (no tracer provider, settings unavailable, anything) and the end path ends the span from its own `finally` even if annotating it threw — an unended span is a leak in the batch processor. A `None` span is a no-op. Nothing in this path can raise into an agent call.

**Exporter contract this depends on** (read off the installed `azure-monitor-opentelemetry-exporter` **1.0.0b56**, `export/trace/_exporter.py`, not assumed): a CLIENT span is exported as `RemoteDependencyData` (the `AppDependencies` table); `gen_ai.system` sets `DependencyType` to `GenAI | {value}` via `_GEN_AI_ATTRIBUTE_PREFIX` (line 120) and is applied *after* the HTTP/DB/messaging branches so it wins over them; `peer.service` sets `DependencyTarget` (`export/trace/_utils.py:148`).

**Verified locally, executed rather than asserted:**
- 9 new tests in `tests/test_telemetry.py` (Gap 300 section) install a real OpenTelemetry SDK `TracerProvider` with an in-memory exporter and assert the actual span object. Full file: **23 passed**. Adjacent suites `test_query_tools.py` + `test_agentic_sage.py` + `test_agent_eval.py` + `test_ops_digest.py`: **277 passed**. `ruff check` clean on both changed files.
- `test_llm_dependency_span_converts_to_a_remote_dependency_envelope` runs the recorded span through the installed exporter's own `_convert_span_to_envelope` — the same function a deployed container runs on export — and asserts `base_type == "RemoteDependencyData"`, `type == "GenAI | az.ai.openai"`, `target == "oai-invoicellm-dev.openai.azure.com"`, `name == "chat gpt-5-mini"`, `success is True`, `properties["agent_name"] == "sage.plan"`.
- `test_llm_dependency_span_is_a_child_of_the_surrounding_request_span` asserts the parent link, which is the entire point: the emitted row carries the request's `operation_Id` and an `operation_ParentId` pointing at the request span, so "how much of this turn was the model?" is answerable as a join against `AppRequests`.

**Not deployed.** Same blocker as Gap 292's fix — `AppDependencies` in `law-invoicellm-dev` will keep returning zero `openai`/GenAI rows until a backend image carrying this code is deployed. The first live check after that deploy is `AppDependencies | where TimeGenerated > ago(1h) | summarize count() by DependencyType`, which should gain a sixth kind, `GenAI | az.ai.openai`.
...
* **Backend Telemetry & Structured Logging:**
  ...
  * `apps/invoice-be/telemetry.py` → **Gap 300 (fixed 2026-08-24, not deployed):** `resolve_gen_ai_system`, `resolve_gen_ai_peer`, `_start_llm_dependency_span`, `_end_llm_dependency_span`, called from `tracked_llm_call` — one `SpanKind.CLIENT` span per LLM call, which the Azure Monitor exporter writes to `AppDependencies` as `DependencyType = "GenAI | az.ai.openai"`, parented to the FastAPI request span. This is the only thing this module emits that is *not* a `customEvents` row, and the reason is that a dependency-vs-request-duration breakdown is a join with `AppRequests` that no custom event can join. See the 2026-08-24 Gap 300 section above.
