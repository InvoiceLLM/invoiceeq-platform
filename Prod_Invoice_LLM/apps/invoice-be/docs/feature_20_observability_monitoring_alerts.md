...
- `_start_llm_dependency_span(...)` / `_end_llm_dependency_span(...)` → open/close one `SpanKind.CLIENT` span named `chat {model}` (the OTel semconv name for a chat completion) on tracer `invoice_be.telemetry.llm`, carrying `gen_ai.system`, `gen_ai.operation.name`, `gen_ai.request.model`, `peer.service`, `server.address`, plus non-semconv `agent_name`/`tenant_id`/`request_id` — `agent_name` specifically because `DependencyType` collapses every LLM call in the app to one value, and the per-feature-area breakdown this gap exists to unblock needs a grouping key in `customDimensions`. Token counts (`gen_ai.usage.input_tokens`/`output_tokens`, `llm_calls`) are set at close, since they are only known then. On failure the span's status is set to `ERROR`, which is what the exporter reads for the dependency's `Success` column.
