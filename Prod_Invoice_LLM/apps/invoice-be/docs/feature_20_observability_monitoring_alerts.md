...
* **Monitoring, Visual Dashboards & Alerting (IaC):**
  ...
  * `infra/modules/monitoring/alert-rules.bicep` → adds Dead-Letter Queue alert (`ApproximateMessageCount > 0` on `extraction-tasks-deadletter-queue`) and `ca-invoice-website` 5xx error alert.
...
- [x] **Task 19.6: Website Diagnostics & DLQ Alert Rules** — Added `ca-invoice-website` to diagnostic settings and 5xx alerts in `09-monitoring.bicep` and `alert-rules.bicep`; added Sev 1 Dead-Letter Queue (DLQ) poison message alert rule in `alert-rules.bicep`.
...
- [x] **Task 19.9: CI/CD Post-Deployment Verification Gate** — Added `verify-deployment` job in `.github/workflows/deploy-prod.yml` to automatically verify public ingress and container app `Succeeded` provisioning state after release.
