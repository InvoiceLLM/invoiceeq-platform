...
| Blocker | Detail |
|---|---|
| CI `benchmark-gate` step is broken | `deploy-dev.yml`'s gate starts the job with `az containerapp job start --command /bin/sh -c --args "…"`, which fails with `unrecognized arguments: -c` (argparse `nargs='+'` rejects a hyphen-prefixed value; reproduced with `--debug`, independent of arg order/Windows path mangling). `--yaml` override silently no-ops on az CLI 2.88.0. `deploy-backend`/`deploy-worker` are gated on `benchmark-gate.result == 'success' \|\| 'skipped'`, so this **blocks all backend/worker deploys**, not just this feature. Working alternative proven live: direct ARM `az rest --method post …/jobs/{name}/start?api-version=2024-03-01` with an unwrapped `StartJobExecutionTemplate` body **including the full persisted `env` array** (a bare override drops env/secrets). No gap filed for this yet. |
...
- `[ ]` Unblock deploys: fix `deploy-dev.yml`'s `benchmark-gate` invocation
