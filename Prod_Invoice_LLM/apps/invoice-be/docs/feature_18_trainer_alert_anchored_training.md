...
* [agents/extraction_agent.py](file:///c:/Users/S%20Banerjee/Desktop/Invoice_LLM/Prod_Invoice_LLM/apps/invoice-be/agents/extraction_agent.py) → `build_multimodal_prompt()`, `build_outbound_multimodal_prompt()`, `extract_node()`, `verify_node()` (now reads `state["rules"]`). **Updated 2026-08-21 (Gap 283)**: `build_outbound_multimodal_prompt()`/`extract_node()`/`verify_node()` used to live in `agents/outbound_extraction_agent.py`; there is one shared graph now, so the tolerance/threshold/severity parameterization this feature added is applied once and reaches both flow directions by construction rather than by being duplicated.
* [agents/outbound_extraction_agent.py](file:///c:/Users/S%20Banerjee/Desktop/Invoice_LLM/Prod_Invoice_LLM/apps/invoice-be/agents/outbound_extraction_agent.py) → `run_outbound_extraction_agent()` only (thin wrapper since Gap 283 — defines no nodes)
...
| `agents/extraction_agent.py` (both flow directions since Gap 283) | `extraction_failed`, `token_limit_exceeded`, `missing_required_field` (OUTBOUND direction profile only) |
...
> **Documented follow-up, not a silent gap.** The five `*_not_verified_in_source` types (`total_`, `line_item_`, `subtotal_`, `unit_price_`, `tax_amount_`) are **explicitly excluded from the "unnecessary alert" tolerance path in this pass**. They ask a verbatim-presence question ("does this figure appear in the OCR text?") that has no numeric band to widen — there is nothing a tolerance form could adjust. They are exposed as `TOLERANCE_EXCLUDED_SOURCE_TEXT_TYPES`, returned under `toleranceExcluded` by `GET /trainer/alert-types`, and the tolerance endpoint 400s on them with the registry's own explanation rather than accepting a write that would do nothing. **Resolution (Gap 229):** Fuzzy verbatim matching is implemented in `utils/verification_tools.py` (`_number_found_in_text`). It normalizes both the target figure and the raw OCR text by stripping currency symbols, thousands separators, and parentheses, comparing them as floats within a 0.01 tolerance (ignoring spaces unless they form a valid thousands separator, and avoiding newline-spanning candidate matches). Duplicates, failures and timeouts are excluded permanently: they report facts, not thresholded judgements.
