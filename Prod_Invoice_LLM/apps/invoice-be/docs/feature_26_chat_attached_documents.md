...
| H6 Tier 3 | `[x]` | — | **Built run 2** (`30dce18`): `_tier3_candidates()` + `TIER3_CANDIDATE_LIMIT` in `services/document_comparison.py`, fired from BOTH the empty-tier-2 branch and the missing-party/date early return — V-12 caught that the second call site was the one that mattered, since tier 2 returns early on exactly the case tier 3 exists for. `tests/test_tier3_discovery.py` → 11 |
| H6b `compare_documents()` + L1–L3 matcher | `[x]` | — | **Built run 2** (`7abe5d3`/`f5e1a6d`/`900105a`): `compare_documents(doc_a, doc_b, mode)` with four modes, `_match_lines()` L1/L2/L3, `resolve_comparison_mode()`, `reconcile_referenced_documents()` (B8, five outcomes). `tests/test_compare_documents.py` → 31 |
| H7 async wiring | `[x]` | — | **Built run 2** (`7674c0d`): the `and payload.attachment_id is None` force-sync clause is removed. `ENABLE_ASYNC_CHAT_QUEUE` deliberately **not** flipped (standing ruling) — Gap 390 records that a `.env` setting it true made six tests 202-vs-200, so the sync path is now pinned per-test rather than assumed |
| H8 `scripts/sweep_chat_attachments.py` | `[x]` | — | **Built run 2** (`84e3a85`). `expires_at IS NULL` means KEEP, deliberately; follows `delete_attachment_chunks()`. `tests/test_sweep_chat_attachments.py` → 7 |
| H9 `infra/chat-doc-ttl-job-only.bicep` | `[~]` | **400** | **Written and DEPLOYED 2026-09-03.** `az containerapp job show` → `caj-chat-doc-ttl-dev`, `Succeeded`, Schedule, `0 5 * * *`, timeout 1800, args `--limit 500`. Then `az containerapp job start` **failed**: the ACR image predates `84e3a85` and does not contain the script. `job show` proves the resource; only an execution proves the job — see Gap 400 for the closing condition |
...
| H15 deferral gaps (B3/B4, RAG raw chunks) | `[x]` | 396–400 | Filed as found, in the same change as the code, throughout runs 2 and 3: **390** (async-queue env), **395** (Azure jailbreak filter blocking the content branch), **396** (German transliteration), **397** (batch rollback), **398** (document audit trail, open), **399** (stuck-Document sweep, deferred + anchored), **400** (stale ACR image) |
| V §P2.10 against Postgres + Redis | `[~]` | — | **Part 2 now has recorded Postgres runs**: V-27 (the answer contract reaching the browser, run 2), the R3 migration and R4 runs (evidence files 01–03), and run 3's `tests/test_document_delete.py` → 11 on real Postgres. Still open: the ≥7-day soak and the human end-to-end pass, which are B11's removal criterion (d)/(e), not build tasks |
