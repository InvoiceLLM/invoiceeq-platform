# Feature 6 (SAGE) — fix history and retrieval-hardening record

**Split out of [feature_6_rag.md](feature_6_rag.md) on 2026-09-17.** That file had grown to 2,120 lines in four layers that
explicitly overrode one another, so a reader could not tell which paragraph described the running system. The spec now keeps
only what is true today; everything below is the record of how it got there and is **archival — do not read it as current
behaviour**. Where this file and the spec disagree, the spec wins.

Contents, in the order they were written:

1. **Dated fix entries, Aug 12 – Sep 1 2026** — per-gap narratives for Gaps 219, 237–242, 253, 294, 306, 310, 313, 315, 365,
   and the Feature 21 SAGE-orchestrator experiment that was built, benchmarked and deleted.
2. **Retrieval hardening (former Feature 6.1, merged 2026-09-03)** — the seven-item analysis, the ranking by value ÷ cost,
   the founder's execution order, and the challenge to the data behind it.
3. **Execution record** — results for A1–A3, B1–B2, C1–C4, BE Gap 569, and the attachment-intent / amount-owed probe of
   2026-09-06 (Gaps 470, 472, 473).

The chat production-readiness work of 2026-09-15 → 17 (BE Gaps 570–611) is **not** here: it is recorded in
[be_features_tracker.md](be_features_tracker.md) and its outcome is described in the spec's "Current algorithm" section.
...
Moved back to the spec on 2026-09-17 — it describes how to verify the running system, not how it was built. See [feature_6_rag.md](feature_6_rag.md) §Verification Plan.
