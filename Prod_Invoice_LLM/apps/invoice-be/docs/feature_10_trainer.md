...
2. `trainer_chat()` (`POST /trainer/sessions/{id}/chat`) — calls `agents/trainer_agent.py::run_trainer_agent()`, which calls `refine_constraints()` (one LLM call turning `current_constraints + user_message` into an updated `List[str]`, falling back to just appending the raw message if the LLM call fails) and then re-runs `run_extraction_agent()` with those constraints as `rules`. **P0 fixes (Jul 19, 2026)**: `refine_constraints()` called `get_llm(temperature=0.0)`, but `get_llm()` no longer accepts `temperature` — every trainer correction crashed with a `TypeError`. Also, `ConstraintList` was missing `model_config = {"extra": "forbid"}`, the same OpenAI strict-schema rejection found in `InvoiceExtractionSchema` (see `feature_2_pipeline_extraction.md`). Both fixed.
...
* **Gap 49 — `trainer_commit()` race condition + non-atomic writes (fixed Jul 24, 2026)**: (1) creating a tenant's first-ever Global template had no `IntegrityError` handling — two concurrent Global commits racing to insert would let the partial unique index reject the loser as a raw, unhandled 500. Fixed: catches `IntegrityError`, rolls back, returns `409` with a clear "refresh and re-apply" message. (2) `trainer_commit()`/`rollback_template()` each did two separate commits (template row, then `ExtractionTemplateVersion` history row) — a crash between them could leave a version bump with no matching history row. Fixed by batching both writes into one commit.
* **Gap 48 — committed rules never reached the Chat agent (fixed Jul 25, 2026)**: committing a rule here only ever affected future extraction; `agents/query_agent.py` had no connection to `ExtractionTemplate` at all. Now Global-scope rules are injected into Chat's SQL/RAG prompts too — see `feature_6_rag.md` for the fix and `be_features_tracker.md` Gap 48 for the full writeup. Vendor-scope rules aren't wired into Chat yet (the question's target vendor isn't known until after routing runs) — tracked as `be_features_tracker.md` Gap 52.

### Recent Fixes (Aug 12, 2026)
* **Gap 212 — `refine_constraints()` fails closed instead of storing raw chat text as a rule (fixed Aug 12, 2026)**: the fallback described in "Current Implementation" step 2 above ("falling back to just appending the raw message if the LLM call fails") survived the redesign and applied on *two* paths — an `except Exception` around the LLM call, and a non-exception path where the structured response carried no `constraints` field. Either way `current_constraints + [user_message]` was returned, so a correction like "Remove the rule requiring PO prefix" typed during a provider blip became a literal new extraction rule for that vendor. Both paths now raise `agents/trainer_agent.py::ConstraintRefinementError` with `REFINEMENT_FAILURE_MESSAGE`, leaving `current_constraints` byte-for-byte unchanged; `run_trainer_agent()` propagates it (no re-extraction runs), and `routers/trainer.py::trainer_chat()` catches it ahead of its generic handler and returns `502` with that message. Because the failure short-circuits before `save_session()`, the session keeps its previous rules *and* does not persist a user turn with no answer. The FE needed no change — `app/trainer/page.tsx`'s existing `catch` already toasts "Failed to process your correction. Please try again." on a non-2xx.

* **Gap 213 — the chat answer cache is now flushed on every scope's commit/rollback, not just Global (fixed Aug 12, 2026)**: Gap 48 (above) added `_invalidate_chat_answer_cache()` and wired it into both `trainer_commit()` and `rollback_template()`, but only inside their Global-scope branches (`if scope == "global"` / `if template.vendor_name is None`). An Existing Vendor or New Vendor commit therefore left `chat_answer_cache:{tenant_id}:*` untouched, and Chat could keep serving a pre-correction cached answer for that vendor's questions for the rest of the 1hr `CACHE_TTL_SECONDS`. Both call sites now invoke it unconditionally, after the re-audit decision. No cache TTL or key-shape change was made, deliberately: `agents/query_agent.py::_cache_key()` builds `chat_answer_cache:{tenant_id}:{normalized_query}` with no vendor dimension, so there is no narrower key set a vendor-scoped commit could target — the tenant-wide flush is both the correct granularity and the only one available. Making the cache vendor-partitioned would be a separate redesign and is out of scope here.
