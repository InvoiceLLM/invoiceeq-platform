...
| Chat-eval test coverage — port the regional question banks into the shipped bank | F23 | senior-dev | `[x]` | Done 2026-08-24. Case set **20 → 35**: 15 cases, 5 each from `tests/{india,us,eu}/chat_question_bank.md`, ported into `benchmarks/` (not `tests/`) so they actually ship and run in `caj-benchmark-eval-dev`. Each region's 9 invoices re-seeded as ordinary rows in the harness's in-memory SQLite (new `benchmarks/region_seed_fixtures.py`, 27 rows + 6 document chunks) — **no Postgres, no Chroma, `MOCK_EMBEDDINGS=true` still holds**. Seeded under **separate tenant ids** because merging them into the base tenant would have falsified four existing reference answers (`TSD-620458` exists in both tenants with different totals; the US tenant has three freight vendors; two regional invoices are dated May 2026). New coverage the old 20 structurally could not reach: OUTBOUND rows, `sa_alerts`/`AUDIT_REQUIRED`, real tax regimes (GST slabs, RCM, intra-EU reverse charge, resale exemption, mixed VAT), and a genuinely NULL `due_date`. **Not ported, stated honestly**: all multi-turn follow-ups (harness has no session state — one turn per case, fresh `session_id`), CGST/SGST-split and GSTIN/VAT-ID questions (no such column on `invoice`), duplicate incidents already covered by an existing case, and the 25-question `realworld_tenant`/NovaTech bank (inbound-only USD, adds no region or direction). Verified by a **real 15-case run against live gpt-5-mini** — 15/15 turns, 0 errors, pass rate 0.333 vs the base set's 0.35, and **persona scored on 15/15 turns** (it is NULL on most base-tenant turns; this is the domain-knowledge bank the component-scoring entry has been missing). Three genuine findings, all unfixed: **new Gap 306** (rule 6b's four-column OR group generated with `items` dropped, twice, on two tenants — two real invoices reported "not found", with faithfulness and relevance both 1.0 and blind to it), a document-grounded "why" answered from the structured row instead (faithfulness 1.0 — everything true, none of it the answer), and **Gap 294 reproducing** on `eu_mixed_vat_rates` with a raw tenant UUID in the user-facing reply. Operational note: the nightly job is now ~70 turns (`--paths default,sage`) against a 5400s timeout sized for 40 |
...
**Wave 4 status, same day (2026-08-24)** — Gaps 302 and 303 are now built; see the table below.
Gap 303's drift-detector half was split out as **Gap 307** rather than left making 303 look
half-closed, because reconstructing a thread and judging one are different pieces of work and only
the first was in scope. **Correction while here**: the note that `infra/monitoring/ai_control_tower.workbook.json`
was "built 2026-08-21, not deployed" is stale wherever it appears — that file was **deleted** in
commit `bd6a255` and does not exist. `infra/monitoring/` holds `cost_health_workbook.json`,
`llm_cost_rollup_nightly.kql`, `llm_cost_by_tool.kql` and the new `chat_thread_sessions.kql`.
There is no AI Control Tower workbook in the repo, so **nothing renders any of Gaps 300/302/303/304/305's
telemetry today** — every one of them is an emitter with no consumer, which is a real and stated
limitation of this whole wave rather than a per-gap footnote.

## Wave 4 — Trace and Thread (2026-08-24)

| Task | Feature | Owner | Status | Notes |
|---|---|---|---|---|
| Gap 302 — Trace-level capture (`chat_turn` event) | F23 | senior-dev | `[x]` | Done 2026-08-24, **not verified live**. One `customEvents` row per turn, emitted by `routers/chat.py::post_chat_message()` and `queue_worker/handlers.py::handle_process_chat_job()` after their commit, on **every** outcome including declined, errored and cache-hit turns — the three that previously emitted nothing at all. Per the founder's decision it carries **real content**: the actual generated SQL and the actual tool output (SQL results table / RAG chunk text), truncated at 3000/12000 chars — the same budgets `services/agent_eval.py` already uses for the judge prompt, asserted equal by test. `telemetry.py` gained `ChatTurn`/`chat_turn_scope()`/`track_chat_turn()`; `run_query_agent()` became a Trace wrapper over a renamed `_run_query_agent()`. No change was needed in `agents/sage_orchestrator.py` — it always returned `stop_reason`/`tool_calls_made`/`tools_called` and every caller discarded them. |
| Gap 303 (a) — thread position on the turn event | F23 | senior-dev | `[x]` | `turn_index` + `seconds_since_prev_turn`, from `query_agent.py::_session_turn_position()`. Counts **assistant** messages so the index is identical on the sync and queue paths. Absent stays absent on a first turn rather than becoming 0. |
| Gap 303 (b) — 30-min-idle session analytics | F23 | senior-dev | `[x]` | **Pure KQL derivation, no new scheduled component and no new Azure resource** — `infra/monitoring/chat_thread_sessions.kql`, using `row_window_session(timestamp, 1d, 30m, session_id != prev(session_id))`. The 6h `ops_digest_job.py` genuinely could not carry a 30-minute cutoff; a new `Microsoft.App/jobs` was rejected because the three needs (session length, abandonment, turns/session) are a `summarize` over data already emitted, against a new job's bicep + image pull + deploy + Gap 298's Stage 8 blocker. Limitation stated: an *alert rule* on these queries is only as fresh as its own evaluation frequency. |
| Gap 304 — judge/queue correlation-ID defect | F23 | senior-dev | `[x]` fix only | **A live defect in code shipped 2026-08-24 morning**, not a new feature: `ThreadPoolExecutor.submit()` copies no contextvars, so every `eval.combined_soft`/`eval.persona` event and every queued turn's `llm_agent_call` landed with `trace_id=""`/`tenant_id=""`/`request_id=""`. Fixed with `utils/logging_config.py::correlation_context()` + explicit `trace_id=`/`request_id=` params, matching `main_worker`'s pattern. **Behaviour change**: worker-path events now carry a real trace id where they carried `""`. Gap 304's own status is unchanged — this is a correctness fix to half (2)'s telemetry. |
| Gap 307 — context-drift detection | F23 | — | `[ ]` | Newly filed, split out of Gap 303. Not started. Now has strictly more to work with than when 303 was opened: turn N's and turn N-1's real SQL and tool output are both queryable from one event stream. |
| Gap 305 — `emit_online_signals()` scheduled caller | F23 | senior-dev | `[~]` | **Not touched by Wave 4** — a parallel task wired it into `ops_digest_job.py` the same day. Wave 4's "fold it into the new job" contingency never applied because no new job was built. |
