...
### Model-swap gate (Gap 466, 2026-09-05)

`tests/run_extraction_harness.py` is the gate a candidate model must pass before it replaces `gpt-5-mini`. It still uploads the 27 `InvoiceEQ_Ground_Truth.xlsx` PDFs through the real API and never edits the ground-truth workbook; what is new:

* **Scoring.** `score_row()` per invoice: `Actual Tax` and `Actual Total` within ±$0.01 of the expected, `Actual Status` case-insensitive equality, `Actual Alert Type` as a set (`None`/`-`/blank all mean "no alerts"). An `ERROR` row fails every field. `SCORED_FIELDS` is the list to extend when the Invoice row gains a `subtotal` column.
* **Outputs.** A `Scores` sheet in `InvoiceEQ_Actual_Results.xlsx`; `tests/InvoiceEQ_Scores_<label>.json` with `invoice_accuracy_pct`, `field_accuracy_pct`, `per_field`, `latency_s` (p50/p95/max/mean), `llm_calls`, `tokens_in/out`, `cost_usd` (registry list price via `utils/model_registry.cost_usd`), `models_seen`, and the `registry_snapshot()` in force.
* **Token capture.** A logging handler on `invoice_be_telemetry`/`invoice_worker_telemetry` collects `llm_agent_call` events emitted in the harness's process. When extraction runs inside the backend process, `cost_note` states that tokens were not captured; read them from the AI Control Tower workbook for the run window instead.
* **Gate.** `--min-accuracy N` exits 1 when field accuracy is below N%. `--label` names the score file (default: the primary deployment). Baseline first (`--label gpt-5-mini`), then each candidate with the backend restarted under that candidate's `AZURE_OPENAI_DEPLOYMENT_NAME`.
* **Not yet run.** No baseline has been recorded as of 2026-09-05 — it costs real tokens and needs the backend up with `ALLOW_MOCK_AUTH=true`.

Note on the api-version paragraph above: since Gap 465 the value is `2024-10-21` (GA) everywhere, not `2024-02-15-preview`.

### Model benchmark matrix (Gap 466 step 3, 2026-09-05)

`scripts/run_model_matrix.py` runs the five comparison tasks for every candidate deployment and writes `docs/extraction_benchmark/runs/matrix-<utc>/` (one JSON + one log per model×task, `matrix.json`, `summary.md`):

| task | runner | metric | input |
|---|---|---|---|
| (a) extraction | `tests/run_extraction_harness.py --deployment <m>` | field accuracy %, invoice accuracy %, line-item P/R/F1, $/invoice, p95 | the 27 InvoiceEQ PDFs (real DI OCR, in-process extraction; backend on :8000 only takes the upload) |
| (b) doc-type | `scripts/run_doctype_matrix.py --deployment <m>` | accuracy % over `DOC_TYPES` | 24 PDFs under `tests/fixtures/doc_types/`, pypdf text so every model sees identical input |
| (c) text-to-SQL | `scripts/run_agent_eval.py --provider azure --model <m> --no-multi-turn` | exec-correct % (`expected_invoice_numbers ⊆ fetched`), mean generation attempts (from `llm_events`), p95 | SQL-route cases of `benchmarks/agent_eval_golden_sample.py` |
| (d) chat | same run as (c), graded by the configured `judge` role | judge accuracy mean, faithfulness, relevance, passed %, $/turn, p95 | all golden cases |
| (e) judge stability | `scripts/rescore_agent_eval.py --run <baseline chat json> --judge-deployment <m>` | mean abs Δ accuracy vs the baseline's judge, pass-flip % | the **baseline's recorded answers** — answers are never regenerated, so only the judge varies |
