...
* `evaluate_support_query(message: str, history: list[dict] | None = None) -> dict` — **the only public entry point.** Returns `{answer, suggest_escalation, low_confidence, escalation_context}`. Resolution order, solution-first by design, **4 steps as of Gap 254** (was 5 briefly — a history-based follow-up step was built and then removed, see below): (1) empty input → SAGE greeting, no escalation; (2) score the query against every `KNOWLEDGE_TOPICS` entry via `_score_topic()` and return the highest-scoring topic's `guidance` with **no** escalation — a question the knowledge base can answer never becomes a ticket; (3) `ERROR_TRIGGERS` regex match → that trigger's message plus `suggest_escalation=True` and a fully populated `escalation_context`; (4) an explicit human-help request (`human|agent|raise ticket|support ticket|contact support|speak to someone|talk to human`) → escalate as `TECHNICAL_SUPPORT`/`NORMAL`; (5) fallback → an honest "I couldn't find a specific help article" with `suggest_escalation=False` **and `low_confidence=True`**.
  ...
  * **`history` is accepted and deliberately unread — descoped, not merely unimplemented (Gap 254 → Gap 256).** The earlier wording here ("accepted for interface stability but not currently read") understated it: an implementation *was* written during Gap 254's first pass and was **removed**. What arrives is only `{role, content}` pairs — `routers/support.py::SupportChatRequest.history`, populated by `SupportChatWindow.tsx` — with no topic metadata anywhere in the request, the response or any store, so there is nothing that identifies "the previously matched topic". The removed implementation approximated it by re-matching the assistant's own prior guidance text against `KNOWLEDGE_TOPICS`, which resolves to whichever topic shares the most vocabulary with the previous answer rather than to that answer's own topic — the guidance blobs are keyword-dense (the Auditor article alone contains "duplicate", "confidence", "review", "paid", "status"). Doing it properly needs `SupportChatResponse` to echo a `topic_id` and the FE to carry it forward: an API contract change plus a frontend change. Tracked as **Gap 256**; `tests/test_support.py::test_history_is_accepted_but_deliberately_unread` pins that passing `history` provably does not change the answer, so this cannot silently regress into a half-working heuristic.
...
* Request/response models: `ContactInquiryRequest`, `AppTicketRequest`, `SupportChatRequest`, `SupportChatResponse` (gained `low_confidence: bool = False` in Gap 254), `TicketResponse`.
