...
- [x] **Task 8.1.2:** Built 2026-07-29 — `routers/outbound_dashboard.py::get_outbound_dashboard_metrics()`, mirroring `dashboard.py::get_dashboard_metrics()`'s SQL-aggregate approach (real SUM/COUNT/GROUP BY for totals/status-breakdown/top-customers; a narrow 4-column scan only for `verification_accuracy`/`average_days_to_payment`, same as inbound's `extraction_accuracy`/`average_processing_time`). `outstanding_receivables` sums `SENT`/`NEEDS_REVIEW`/`UPLOADED` per this doc's spec; `at_risk_receivables` reuses Feature 7.1's read-time overdue definition (`SENT` + `due_date < today`) directly as a SQL condition, no persisted status. `average_days_to_payment` excludes any invoice missing either `sent_at` or `paid_at` from the average rather than estimating, same honest-average rule as the inbound metric. Zero edits to `dashboard.py`. 4 new tests in `tests/test_outbound_dashboard.py` (totals/breakdown/top-customers, verification-accuracy + days-to-payment math incl. a missing-`sent_at` exclusion case, customer/status filtering, empty-tenant zeros) — full suite 124/124 passing.
