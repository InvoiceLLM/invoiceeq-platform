...
WHAT STILL GUARDS OUTBOUND, so this is a removal and not a hole:
    ...
    confirm-send              can_audit
    mark-paid                 can_audit, and only from status SENT
The person who prepares an invoice still cannot be the one who issues it.

DEPLOY ORDER. The code that stops reading this column ships in the same release
as this migration, and `entrypoint.sh` runs `alembic upgrade head` before the
new container serves traffic. Azure Container Apps keeps the previous revision
serving while that happens, so there is a short window where old code is live
against the new schema. Old code SELECTs `users.*`, so during that window user
lookups on the old revision will fail until traffic shifts. Deploy at a quiet
moment, or land the code release first and this migration second.
