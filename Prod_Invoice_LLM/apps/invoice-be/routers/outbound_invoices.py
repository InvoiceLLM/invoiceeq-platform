...
from dependencies import (
    ...
    # BE Gap 721: confirm-send only -- the same rule for humans, stricter for
    # keys on the "Full Automation except sending" policy.
    require_send_scope,
    TenantContext,
)
...
async def confirm_send_outbound_invoice(
    ...
    #
    # BE Gap 721: this is the one route on `require_send_scope` rather than
    # `require_actions_scope`. The two are identical for humans; they differ for
    # a key on the "Full Automation except sending" policy, which is refused
    # here and nowhere else. Mark-paid below deliberately keeps the ordinary
    # gate -- it only accepts an invoice already in SENT, so a human has
    # necessarily signed the send off before a key can record the payment.
    context: TenantContext = Depends(require_send_scope),
    db_session: Session = Depends(get_db_session),
):
