...
* **P0 LLM Temperature Parameter Error (Resolved Jul 19, 2026)**: Fixed unsupported temperature parameter in `agents/extraction_agent.py` line 127. The `get_llm()` call used `temperature=0.0` but some LLM models don't support custom temperature values, causing error: `Unsupported value: 'temperature' does not support 0.0 with this model. Only the default (1) value is supported.` Removed the temperature parameter to use model defaults.
