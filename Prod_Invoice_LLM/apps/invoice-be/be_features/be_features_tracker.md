...
- `[~]` **Gap 33: Extraction faithfulness on internally-inconsistent totals** — found and mitigated Jul 21, 2026, not fully closable by a prompt fix alone. The model occasionally "corrects" a printed total that doesn't arithmetically reconcile instead of transcribing it as-is, silently defeating the tax-mismatch guardrail. Mitigated by tightening `grand_total`'s field description; observed ~1-in-5 residual failure rate in repeat testing after the fix. This is inherent LLM stochasticity, not a deterministic bug — track its real-world frequency via the Feature 13 benchmark's ongoing pass rate rather than treating as closed.
