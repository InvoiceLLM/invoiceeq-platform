...
- [x] **Task 2.4: Build AI Team Section** — backfilled 2026-07-28, found already built and undocumented.
  ...
  - **The agent cards are presentational, not interactive** (clarified 2026-08-05, Gap 158). `AgentCard` carries a mouse-move 3D tilt and hover glow, but no `onClick`, `Link` or `href` — and never had one. It previously also carried a `cursor-pointer` class, which advertised a click target that did not exist; that class was removed rather than a click interaction added, so the cursor now reads `auto` over the cards and the hover tilt remains the only affordance. Verified in-browser: computed `cursor` is `auto` on all agent cards.
