...
* **Edited:** `infra/08-apps.bicep` —
  ...
  - New var `publicOrigin` — `customDomainName` when set, else the existing `websiteFqdn` (CAE domain). Feeds `googleRedirectUri`, ~~`salesforceRedirectUri`~~ (the Salesforce connector was removed 2026-08-28, Gap 334 — the bicep param itself is untouched, since `infra/` is deliberately out of that Gap's scope, but it now feeds nothing), and `backendApp`'s `backendPublicUrl`/`publicAppUrl`/`frontendUrl` params — all switch wholesale to the custom domain once set (OAuth redirect URIs, `BACKEND_PUBLIC_URL`, `PUBLIC_APP_URL`, `FRONTEND_URL` are each a single value in this codebase, not a list, so "prefer custom domain, fall back to CAE FQDN" is the only sane shape — see Gap 131 for the precedent of website-FQDN-based redirect URIs).
...
2. **Cutting over once a domain is purchased**, in order (full detail with exact record names/values in `THIRD_PARTY_INTEGRATIONS_SETUP.md` §6):
   ...
   6. **OAuth app updates** (manual, Google Cloud Console, following the exact pattern already used for Gap 131): update the registered redirect URI to `https://<domain>/api/connectors/callback/google_drive` — the bicep-side values already switch automatically once `customDomainName` is set (see File Coordinates above); only the third-party dashboard registration needs a manual edit to match.

      > ~~**Salesforce Connected App** callback URL cutover~~ — **struck 2026-08-28, Gap 334: this outstanding manual step no longer exists.** The Salesforce connector was removed (cross-org OAuth structurally blocked by an External Client App with Distribution State = Local, plus a wrong data model — it browsed Libraries/`ContentWorkspace`, while real invoices live on Account/Opportunity records). There is no Salesforce Connected App to update at cutover time. Do not treat this as a pending task.
