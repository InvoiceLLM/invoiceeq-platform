...
| `routers/outbound_invoices.py`, `routers/outbound_audit.py`, `routers/outbound_dashboard.py` (incl. its own `list_outbound_invoices()`), `routers/settings.py` | new | Zero edits to `routers/invoices.py`, `audit.py`, `dashboard.py` — an earlier draft of this plan proposed parameterizing `GET /invoices` directly; reverted 2026-07-29 to keep this row true |
| FE: `SendInvoiceStatusTable.tsx`, `OutboundAlertConsole.tsx`, `OutboundMetricsGrid.tsx`, `OutboundInvoicesTable.tsx`, `OutboundFilterBar.tsx`, `VendorFlowToggles.tsx`, `app/settings/page.tsx` | new | New components; `DropZone.tsx`/`PdfViewerCanvas.tsx`/`ClientPerformanceChart.tsx` reused unmodified by import |
| `app/ingestion/page.tsx` | existing, edited (small) | Conditional tab header, gated on the new Settings toggles |
| `app/dashboard/page.tsx` | existing, edited | **Revised 2026-07-29 (Dashboard/Audit split, FE Gap 28)**: no longer gains an outbound invoice list — instead loses its *existing* inbound one. Keeps only the conditional metrics-grid split (`grid-cols-2` when both flows on) plus a new shared `NeedsAttentionWidget.tsx`; `RecentInvoicesTable.tsx`/`FilterBar.tsx` and their pagination state relocate to a new `app/invoices/page.tsx` |
| `app/invoices/page.tsx` (new), `components/layout/Sidebar.tsx` (existing, edited) | new / edited | Unified Invoices/Audit queue screen closing FE Gap 28 — relocated `RecentInvoicesTable.tsx`/`FilterBar.tsx` (inbound tab) + new `OutboundInvoicesTable.tsx`/`OutboundFilterBar.tsx` (outbound tab, once built); Sidebar's "Invoices" nav item (removed in Gap 26 for pointing nowhere) re-added, now pointing here |

Full detail for every row: `apps/invoice-be/docs/feature_2.1/6.1/7.1/8.1/16/17_*.md`, `apps/invoice-fe/docs/feature_2.1/3.1/4.1/10_*.md`, `apps/invoice-website/website_features/feature_3.1_vendor_flow_pricing.md`. Dashboard/Audit split detail: `apps/invoice-fe/docs/feature_2_dashboard.md` Task 2.7, `feature_4_auditor.md` Task 4.9.
